...
module.exports = async (app, db, io) => {
  ...
  app.get('/api/v2/boards', async (req, res, next) => {
    try {
      ...
      if (!workspaceUuid) {
        return res.status(400).json({message: 'Invalid workspace ID', success: false, data: null});
      }
      if (!user || !user._id) {
      ...
      if (!workspace) {
        return res.status(400).json({message: 'Workspace not found', success: false, data: null});
      }
      let thisWorksSpaceBoards = workspace.boards;
      let fetchedBoards = [];
      for (let i = 0; i < thisWorksSpaceBoards.length; i++) {
        let board = await db.collection('boards').findOne({_id: thisWorksSpaceBoards[i]});
        if (board) {
  ...
  app.post('/api/v2/boards', async (req, res, next) => {
    try {
      ...
      if (!boardUuid) {
        ...
        let updatedWorkspace = await db.collection('workspaces').findOneAndUpdate({uuid: workspaceUuid}, {$push: {boards: createdBoard.insertedId}}, {returnOriginal: true});
        let boardThatCreated = await db.collection('boards').findOne({_id: createdBoard.insertedId});
        namespace.emit('board', {type: 'create', data: boardThatCreated});
        return res.status(201).json({message: 'Board created successfully', success: true, data: boardThatCreated});
      } else {
        ...
        return res.status(201).json({message: 'Board updated successfully', success: true, data: result});
      }
    } catch (error) {
  ...
  app.post('/api/v2/boards/archive', async (req, res, next) => {
    try {
      ...
      let updatedWorkspace = await db.collection('workspaces').findOneAndUpdate({boards: boardUuid}, {$pull: {boards: boardUuid}}, {returnOriginal: true});
      let result = await db.collection('boards').deleteOne({uuid: boardUuid}, {returnOriginal: true});
      namespace.emit('board', {type: 'delete', data: result});

      return res.status(201).json({message: 'Board archived successfully', success: true, data: {result: result, updatedWorkspace: updatedWorkspace}});
    } catch (error) {
